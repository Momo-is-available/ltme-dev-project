...
export default function Profile({ setShowAuthModal, setPostToSaveAfterAuth }) {
	...
	const loadProfileData = useCallback(async () => {
		...
		try {
			...
			try {
				...
				const userProfile = await apiClient.getUserByUsername(username);
				profileUserData = {
					id: userProfile.id,
			...
			// Load user posts using API client
			try {
				const userPostsData = await apiClient.getUserPosts(
					profileUserData.id
				);

				// Transform to match expected format
				const transformedPosts = userPostsData.map((post) => ({
					id: post.id,
					title: post.title || "",
					caption: post.caption || "",
					imageUrl: post.image_url || "",
					...
					avatarUrl: profileUserData.avatar,
				}));

				setUserPosts(transformedPosts);
				setUserStats({
					totalPosts: transformedPosts.length,
					totalSaves: 0, // Will be updated by useEffect watching savedPostIds
	...
	useEffect(() => {
		const handlePostUploaded = () => {
			...
			if (isOwnProfile) {
				if (import.meta.env.DEV) {
					console.debug(
						"[Profile] Post uploaded, refreshing profile data"
					);
				}
				loadProfileData();
			}
	...
	const loadAlbums = async () => {
		...
		try {
			...
			// Get user albums (backend handles public/private filtering)
			const albumsData = await apiClient.getUserAlbums(profileUser.id);
